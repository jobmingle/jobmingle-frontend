...
import {HiAcademicCap, HiMiniBriefcase, HiBookOpen, HiCog, HiArrowLeft} from "react-icons/hi2";
      ...
      >
         <ul className=" flex flex-row md:flex-col gap-[0.8rem] w-full justify-between md:justify-between items-center md:items-start sora">
            <li className=" md:w-full">
               <Link
                  href="/dashboard"
                  className={`flex gap-2 transition-all 0.3s text-sm lg:text-lg py-3 hover:bg-gray-50 rounded-l px-8 md:px-4 ${isActive("/dashboard") ? "bg-gray-50" : ""}`}
               >
                  <HiMiniBriefcase className="hover:text-yellow-500 text-stone-950 w-[1.8rem] h-[1.8rem] " />
                  <p className="hidden md:flex text-center"> Jobs</p>
               </Link>
            </li>
            <li className=" md:w-full">
               <Link
                  href={"/dashboard/courses"}
                  className={`flex gap-2 transition-all 0.3s text-sm lg:text-lg py-3  hover:bg-gray-50 rounded-l px-8 md:px-4 ${isActive("/dashboard/courses") ? "bg-gray-50" : ""}`}
               >
            ...
            <li className=" md:w-full">
               <Link
                  ...
                  className={`flex gap-2 transition-all 0.3s text-sm lg:text-lg py-3 hover:bg-gray-50 rounded-l px-8 md:px-4 ${isActive("/dashboard/settings") ? "bg-gray-50" : ""}`}
               >
