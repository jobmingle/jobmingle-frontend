...
import siteicon from "@/public/websiteIcon.png";
...
const Page = () => {
   ...
   return (
      <div className={`min-h-[70vh] sm:min-h-[85vh] md:min-h-[90vh] h-auto mx-2 md:mx-0 ${Active ? "max-h-screen overflow-y-hidden" : ""} `}>
         {/* <main className="border w-full relative">{Active ? <Modal setActive={setActive} /> : null}</main> */}
         <main className=" w-full relative">{Active ? <Modal ChosenJob={SelectedJob} setActive={setActive} /> : null}</main>
         <main className=" w-full h-auto min-h-[35vh] md:min-h-[50vh] rounded-md my-10 p-2 lg:p-4">
            <div className=" flex justify-between gap-4 items-center flex-wrap">
               <section className="flex flex-col gap-5">
                  ...
                  <p className="text-sm lg:text-[100%] font-bold montserrat text-cebbter">Here is your job listing statistics from july-september 2023</p>
               </section>
               <section>
                  <input
                     type="date"
            ...
            <div className="flex justify-between mt-10">
               <Button className="w-[160px] max-md:w-[160px] text-[14px] mt-3 md:my-4 text-white sora bg-[#f5cb1a]" onClick={handleListAJob}>
                  List a job here !
               </Button>
            ...
            {AllJobs.length >= 1 ? (
                  ...
                  {AllJobs.map((job) => (
                     <div key={job.id} className="border p-2 rounded-md">
                        ...
                        <div className="flex gap-3 mt-2">
                           <button className="border sora text-green-500 sora text-xs lg-text-sm px-1.5 py-0.5 rounded-sm" onClick={() => handleEdit(job)}>
                              Edit
                           </button>
                           <button className="border sora text-red-500 sora text-xs lg-text-sm px-1.5 py-0.5 rounded-sm" onClick={() => handleDelete(job.id)}>
                              Delete
                           </button>
                        </div>
                        <section className="flex flex-row justify-between m-1 py-1 ">
                           <div className="flex gap-2 items-center">
                              ...
                              <a href={job.website} className="w-6 h-6">
                                 <Image src={siteicon} alt="link_icon" className="opacity-50" />
                              </a>
                           </div>
                           <button className="w-6 h-6">
               ...
               <main className=" w-full h-auto min-h-[35vh] md:min-h-[50vh] rounded-md mt-10 p-2">
                  ...
                  <section className="flex flex-col justify-center items-center py-4">
                     ...
                     <Link href={"/employer-dashboard/list-a-job"} className="sora capitalize text-md text-[#f5cb1a] underline">
